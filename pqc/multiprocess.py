...
class Multiprocess(Interface):
    ...
    def init(self,arr1_base,arr2_base):
        ...
        arr1 = arr1.view(np.complex128).reshape(self._samples, 2**self.qubits)
        arr2 = np.ctypeslib.as_array(arr2_base.get_obj())
        arr2 = arr2.view(np.complex128).reshape(self._samples, 2**self.qubits)
    ...
    def job(self,func,iterable):
        arr1_base = Array(ctypes.c_double, self._samples*2**self.qubits*2)
        arr2_base = Array(ctypes.c_double, self._samples*2**self.qubits*2)
